...
`actor_timeout_seconds` (positivo) limita a execução do ator; `cancel_path` aponta para um arquivo de pedido de cancelamento. Em ambos os casos o runner encerra o processo do ator, grava `phase=INTERRUPTED` com `interruption_reason=TIMEOUT|CANCELLED` no journal e deixa o estado canônico byte a byte inalterado. Nova execução sem `resume_interrupted=true` não inicia ator algum. A retomada explícita exige remover o pedido de cancelamento, valida o estado de origem e reinicia a mesma operação sob lock; qualquer relatório incompleto é descartado. No POSIX, o grupo de processos do ator é encerrado; no Windows, a garantia de encerramento cobre o processo direto, não descendentes independentes.
